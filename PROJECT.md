...
### 5.5 反向清单（明确不做的事）
- ❌ 自建文献索引（用 arXiv + S2 API 就够）
- ❌ Post-train 自研 base 模型
- ❌ 自研 PDF 板式解析模型（用开箱工具即可，不一刀切 OCR vs 非 OCR）
- ❌ DCA 端侧百万长序列
- ❌ 把 KPAX 扩成科研工具（论文检索 / 学术写作 / 文献管理不是我们这条线）

**立场说明**：上面每一条都是**能力约束型**决定——没有对应积累，做不过现成方案。不是战略回避竞争。**有好工具就用，有能力就上正面**。
理由和更长的论证见 `notes/research/` 里的分析笔记。
