...
**一句话**：一个双产品项目——**AXL** 是学术底座（多学科 AI 辩论引擎），**KPAX** 是消费决策工具（调用 AXL 帮用户"把这件事想透"）。

| 产品 | 定位 | 代码位置 | 用户 | 状态 |
|---|---|---|---|---|
| **AXL** | 跨学科推演底座，开源 | `projects/knowledge-graph/` | 研究者 / KPAX 自己 | 产品 87/100，记忆系统 Phase 2 完成 |
| **KPAX** | 消费端决策工具 | `kpax/` | 10 位朋友先行测试 → 扩大 | 骨架完成，v0 形态设计中（座谈会 3D） |

**核心信念**：多 agent 跨学科碰撞 → 涌现创造力（不是效率工具，是能力工具）。Wisland 赛道是 efficiency（省时间），我们赛道是 capability（新能力）。赛道不碰，不追他们工程栈。
...
### 5.5 Ken 不要追赶 Wisland 的 efficiency 赛道（反向清单）
- ❌ 自建文献索引（用 arXiv + S2 API）
- ❌ Post-train 自研 base 模型
- ❌ 自研 PDF 板式解析
- ❌ DCA 端侧百万长序列
- ❌ 科研工具产品功能（论文检索 / 学术写作 / 文献管理）
- 理由见 `notes/research/wisland-analysis-and-positioning.md` C.3 + C.4
